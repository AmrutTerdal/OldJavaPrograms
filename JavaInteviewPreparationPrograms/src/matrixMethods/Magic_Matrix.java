		...
		if (n > 5)
			...
			if (n % 2 != 0) {
				...
				while (k <= n * n) {
					...
					if (i < 0 && j > n - 1) // Condition for the top-right
											// corner element
					{
						i = i + 2;
						j--;
					...
					if (i < 0) // Wrapping around the row if it goes out of
								// boundary
						i = n - 1;

					if (j > n - 1) // Wrapping around the column if it goes out
									// of boundary
						j = 0;

					if (A[i][j] > 0) // Condition when the cell is already
										// filled
					{
						i = i + 2;
						j--;
